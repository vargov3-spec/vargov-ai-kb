...
Слаги стареют: выгрузка аккаунта у конфигуратора собирается вручную (последняя — 01.09.2026), поэтому при массовом переименовании первыми становятся мёртвыми ссылки в его разметке. В базе есть `scripts/check_3ddd_links.py` — выборочная проверка адресов с паузами. **Оговорка, проверено 06.09.2026:** с машины владельца ни 3ddd.ru, ни 3dsky.org не открываются, соединение отваливается по таймауту, поэтому отсюда проверка невозможна и код 000 означает «не дошли», а не «ссылка мертва»; скрипт это различает и выходит с кодом 2. Проверять оттуда, где площадка доступна, либо просить агента конфигуратора — у него работает API аккаунта. Нашли 404 — он присылает свежую выгрузку, база пересобирается в тот же день.
