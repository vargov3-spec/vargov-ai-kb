...
**Главный факт, перепись агента сайта 06.09.2026** (запрос по каждому из 605 артикулов, только свой аккаунт): моделей 4152, без моделей 8 артикулов, ровно одна модель у 80, больше одной — у 517, то есть у 85 % каталога; максимум 51 модель у LC0023, обычно 5–9. Значит прямая карточка `/3dmodels/show/<slug>` — это адрес ОДНОЙ из моделей артикула, а не «модели артикула». Реестр слагов `references/3ddd-models.json` (604 записи, из выгрузки аккаунта конфигуратора) даёт по одному представителю на артикул. Арифметика счётчиков, сверена 07.09.2026: 605 артикулов каталога, минус два без слага (LC0255, LC0257) и минус LC0543-1, у которого своей карточки нет вовсе, — это **602 узла `3DModel`** в графе; в реестре слагов на один больше плюс запись LC0104-1, которой нет в каталоге. Обход конфигуратора идёт по всей выгрузке, поэтому все ссылки базы им накрыты.

**Выгрузка собирается ПО ТЕГУ, и при пустом теге подставляет соседа** (вскрыто 07.09.2026). У дочерних артикулов стоял слаг базового: LC0406-1 вёл на карточку LC0406, LC0192-1 — на LC0192. Обходом ссылок такое не ловится: чужая карточка жива и отвечает 200. Поправки вынесены отдельным слоем `references/3ddd-corrections.json` (правильные слаги, список пустых тегов, список артикулов без своей карточки) — потому что источник, `data-sources/3ddd-own.json` конфигуратора, пересобирается и затирает слепок в репозитории.

**Пустой тег ≠ нет моделей.** По списку всех 5047 слагов аккаунта (выгрузка 01.09.2026) у LC0217-1 шесть карточек, у LC0543-2 — десять, у LC0545-1 — одиннадцать, у LC0406-1 и LC0593-1 — по одной: модели на месте, тег с карточек снят. По-настоящему пусто только у LC0255, LC0257 и LC0543-1. `CollectionPage` снят всем восьми артикулам с пустым тегом (узлы есть у шести из них), вернётся, когда конфигуратор проставит теги.
