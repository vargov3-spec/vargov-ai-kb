...
У бренда 604 собственные карточки на 3ddd.ru (зеркало 3dsky.org) в аккаунте `vargov`. Правило владельца от 05.09.2026: разрешены только свои карточки и аккаунтные списки `3ddd.ru/users/vargov/models?tag=<артикул>`; общий поиск `3ddd.ru/3dmodels?tag=` запрещён — он показывал чужие модели. Проверено: запрещённой формы нет ни на сайте, ни в конфигураторе, ни в базе.

Формы различаются по назначению, и это не расхождение, а разделение:
- **кнопка «Скачать 3D-модель»** — там, где на неё нажимает человек: сайт 559 аккаунтных списков + 44 прямые карточки, конфигуратор 586 + 10. Списки устойчивы к переименованию слага и показывают все варианты артикула (LC0166, LC0166-1 и т. д.);
- **машиночитаемый слой** — прямые карточки `/3dmodels/show/<slug>` в `subjectOf` узла Product: у карточки есть название, автор и превью, у списка — нет. В базе так на 603 артикулах; конфигуратор сделал то же на своих страницах `/sku/` (467 русских и 467 английских, коммит 714023dd от 06.09.2026, выкладка по слову владельца); у сайта в фиде пока стоят аккаунтные списки — предложение отдать там прямые адреса ему передано, решение за ним.

Разделение принято всеми тремя 06.09.2026: кнопка для человека — список (устойчив к переименованию слага, а слаги у нас уже переименовывались массово), разметка для машины — прямая карточка. Слаги во всех трёх слоях из одной выгрузки аккаунта, поэтому разойтись они не могут. У конфигуратора в сборщике стоит проверка: если прямого слага в выгрузке нет, ссылка не ставится вовсе — правило появилось после случая с LC0061, когда в реестре владельца оказалась чужая модель.

Слаги стареют: выгрузка аккаунта у конфигуратора собирается вручную (последняя — 01.09.2026), поэтому при массовом переименовании первыми становятся мёртвыми ссылки в его разметке, а не в базе. Для этого в базе есть `scripts/check_3ddd_links.py` — выборочная проверка адресов с паузами (25 из 603 по умолчанию, `--all` для полного обхода); массовое переименование задевает десятки карточек сразу, и выборка ловит его почти наверняка. **Оговорка, проверено 06.09.2026:** с машины владельца ни 3ddd.ru, ни 3dsky.org не открываются — соединение отваливается по таймауту, поэтому отсюда проверка невозможна, и код 000 означает «не дошли», а не «ссылка мертва»; скрипт это различает и выходит с кодом 2. Запускать там, где площадка доступна, либо просить проверку у агента конфигуратора — у него есть доступ к API аккаунта. Нашли 404 — он присылает свежую выгрузку, база пересобирается в тот же день.

Оговорка про пользу: страницы конфигуратора закрыты от ИИ-краулеров (см. раздел выше), поэтому прямые ссылки в его разметке работают на обычный поиск, а не на языковые модели. Для моделей тот же слой несут база знаний и фид сайта.

Реестр слагов: `references/3ddd-models.json`, 604 записи, собирается из выгрузки аккаунта конфигуратора. Сверен с конфигуратором 06.09.2026 посимвольно — расхождений нет. Без карточки два артикула: LC0255 и LC0257. Запись `LC0104-1` есть в выгрузке аккаунта, но такого артикула нет в каталоге — вероятно, вариант ленты со своей карточкой.
