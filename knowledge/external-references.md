...
### Sketchfab — второй канал 3D-моделей (аудит 07.09.2026)
Профиль: https://sketchfab.com/vargov · страница моделей `/vargov/models`. Полный обход API (`search?type=models&user=vargov`, 37 страниц) дал:
- **868 моделей, 449 артикулов** из 605 каталога; 156 артикулов не залито; лишних артикулов нет;
- **у всех 868 адрес `sketchfab.com/3d-models/none-<хэш>`** — слаг «none», имя при загрузке приходило пустым. Для поиска и ИИ адрес пустой у всего аккаунта;
- **все 868 скачиваемые, лицензия CC Attribution** — геометрию можно скачать и использовать коммерчески. Решение владельца не подтверждено, вынесено ему;
- 177 названий обрезаны ровно на 32 символах («Lighting composition — stone, cr»);
- 187 моделей дублируют название у 139 артикулов (LC0268 — 8 лишних, LC0151 — 8, LC0592 — 7);
- просмотров 1123 на 868 моделей, у 360 ноль, лайков нет — канал не работает;
- в описании профиля указаны материалы («in crystal and glass»), которых владелец обычно не называет.

Чем ценен: в отличие от 3ddd открыт отовсюду (3ddd с машины владельца недоступен), даёт iframe и AR-просмотр, у моделей есть oEmbed. План: починить адреса и названия, добавить в описания ссылки на карточки изделия, поставить Sketchfab второй ссылкой на 3D-модель в графе и профиль в `sameAs`, затем встроить 3D-просмотр в карточки сайта. Направление передано агенту Pinterest 07.09.2026.
