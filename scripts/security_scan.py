...
# Ожидаемые счётчики Метрики: 111091941 — vargov.ru, 110741646 — конфигуратор.
#
# Номер конфигуратора обновлён 08.09.2026 после проверки у владельца счётчика.
# Здесь стоял 89943818 с подписью «счётчик владельца» — и подпись была неверной:
# по списку Метрики его завёл служебный аккаунт Яндекс.Бизнеса под карточку
# организации. Своих целей там не создать, а доступ на просмотр Бизнес раздаёт
# всем, кого привязали к карточке. То есть тревога «чужой аккаунт видит поведение
# посетителей» была справедлива как раз для старого номера, а не для нового.
# Собственный счётчик конфигуратора при этом показывал ноль визитов за квартал:
# на него никто не ссылался, хотя переходов с сайта было 207.
# Смена штатная: коммит 38ac9ee4 в конфигураторе от 30.08.2026, номер лежит в
# одном месте — YM_ID в src/metrika.ts, оттуда подставляется в index.html и в
# статические страницы /sku/. История до 30.08 осталась в 89943818.
EXPECTED_METRIKA_IDS = {"111091941", "110741646"}
